...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 11 18:38:11 2018
...
def tdn(sample_key):
    ...
    """Input Data"""
    main_data = pd.HDFStore("./data/processed/sample_robustness_data/sample" + str(sample_key))
    x_train = main_data['x_train']; y_train = main_data['y_train']
    x_cv = main_data['x_cv']; y_cv = main_data['y_cv']
    x_test = main_data['x_test']; y_test = main_data['y_test']
    ...
    while hh_size <= hh_end:
        ...
        save_name = "./output/Temp/robustness/sample_splitting/tdn/tdn_"+str(sample_key)+"_"+str(hh_size)
    ...
    with np.load("./output/Temp/robustness/sample_splitting/tdn/tdn_"+str(sample_key)+"_"+str(hh_size)+'.npz') as data:
        w_1 = data['w_1']
        w_2 = data['w_2']
        b_1 = data['b_1']
        b_2 = data['b_2']
    ...
    sess.close()
#%% Parallel Loop
def tdn_parallel(sample_key):
    train_accuracy, cv_accuracy, test_accuracy, w_1, w_2, b_1, b_2 = tdn( sample_key )

    np.savez("./output/robustness/sample splitting/tdn/tdn"+str(sample_key),
...
Parallel(n_jobs = number_cores)(delayed(tdn_parallel)(sample_key) for sample_key in sample_range )
         #range(500) )
