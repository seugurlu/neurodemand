...
#%% Define the function that will perform the optimization
def cfnn_dem(sample_key):
    ...
    x_train = raw_data['x_train_dem']; y_train = raw_data['y_train']
    x_cv = raw_data['x_cv_dem']; y_cv = raw_data['y_cv']
    x_test = raw_data['x_test_dem']; y_test = raw_data['y_test']

    """Edit Input Output Data"""
    x_train.iloc[:,:number_goods+1] = np.log( nf.impose_homogeneity( x_train.iloc[:,:number_goods+1] ) )#Generate input matrix as log
    x_cv.iloc[:,:number_goods+1] = np.log( nf.impose_homogeneity( x_cv.iloc[:,:number_goods+1] ) )#Generate input matrix as log
    x_test.iloc[:,:number_goods+1] = np.log( nf.impose_homogeneity( x_test.iloc[:,:number_goods+1] ) )#Generate input matrix as log
        
   #Drop d.hh.size = d.n.child + d.n.adult
    x_train = x_train.drop('d.hh.size',1)
    x_cv = x_cv.drop('d.hh.size',1)
    x_test = x_test.drop('d.hh.size',1)
    
    x_size = x_train.shape[1] #Size of the input layer
    ...
    demographics = main_sample.filter(regex = "^d")
    demographics = demographics.drop('d.hh.size',1)
    mean_demographics = demographics.mean()
    std_demographics = demographics.std()
     
    x_train.iloc[:,number_goods+1:] = ( x_train.filter(regex = "^d") - mean_demographics ) / std_demographics
    x_cv.iloc[:,number_goods+1:] = ( x_cv.filter(regex = "^d") - mean_demographics ) / std_demographics
    ...
    while hh_size <= hh_end:
        ...
        save_name = "./output/Temp/cfnn_dem/coefficients_cfnn_dem_"+str(sample_key)+"_"+str(hh_size)
    ...
    with np.load("./output/Temp/cfnn_dem/coefficients_cfnn_dem_"+str(sample_key)+"_"+str(hh_size)+'.npz') as data:
        w_1 = data['w_1']
        w_2 = data['w_2']
        b_1 = data['b_1']
        b_2 = data['b_2']
    ...
    sess.close()
#%% Parallel Loop
def cfnn_dem_parallel(sample_key):
    train_accuracy, cv_accuracy, test_accuracy, w_1, w_2, b_1, b_2 = cfnn_dem( sample_key )
    np.savez("./output/NN/cfnn_dem/pycoefficients_cfnn_dem"+str(sample_key),
...
Parallel(n_jobs = number_cores)(delayed(cfnn_dem_parallel)(sample_key+1) for sample_key in sample_range )
         #range(500) )
