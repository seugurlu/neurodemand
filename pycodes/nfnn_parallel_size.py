...
import nfnn_functions_size as nffn
...
def parallel_nfnn (sample_key):    
    ...
    for sample_size in [1000, 2000, 3000, 4000]:
        ...
        x_train_sample = np.log( x_train_sample )#Generate input matrix as homogeneity adjusted log
    
        cv_accuracy, test_accuracy, w_1_out, w_2_out, b_1_out, b_2_out, hh_size = nffn.main_with_cv(sample_key+1, x_train_sample, y_train_sample, x_cv, y_cv, x_test, y_test, tol_iter, number_goods)
        np.savez(os.getcwd()+"/output/NN/Size/pycoefficients_nfnn_"+str(sample_key+1)+'_'+str(sample_size), w_1 = w_1_out, w_2 = w_2_out, b_1 = b_1_out, b_2 = b_2_out, 
