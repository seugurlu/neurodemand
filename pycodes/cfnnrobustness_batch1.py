...
import neural_functions_robustness as nf
...
def cfnn(sample_key):
    ...
    while hh_size <= hh_end:
        ...
        '''Variables'''
        w_1 = tf.Variable(tf.random_normal([x_size, hh_size], stddev = 0.1), dtype = tf.float32)
        w_2 = tf.Variable(tf.random_normal([hh_size, y_size], stddev= 0.1), dtype = tf.float32)
        ...
        train_step = tf.train.MomentumOptimizer(learning_rate=0.01, momentum=0.9, use_nesterov=True).minimize(train_cost)
        ...
        save_name = "./output/Temp/cfnn/robust/coefficients_cfnn_"+str(sample_key)+"_"+str(hh_size)
    ...
    with np.load("./output/Temp/cfnn/robust/coefficients_cfnn_"+str(sample_key)+"_"+str(hh_size)+'.npz') as data:
        w_1 = data['w_1']
        w_2 = data['w_2']
        b_1 = data['b_1']
        b_2 = data['b_2']
...
def cfnn_parallel(sample_key):
    ...
    np.savez("./output/NN/cfnn/robust/pycoefficients_cfnnrobust"+str(sample_key),
