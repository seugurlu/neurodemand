...
def main(sample_key, x_train, y_train, x_cv, y_cv, x_test, y_test, tol_iter, number_goods):
    ...
    cv = 1 #Will cross validation be done?  
    ...
    if cv == 1:
        ...
        while hh_size <= hh_end:
            ...
            save_name = os.getcwd()+"/output/Temp/Size/coefficients_nfnn_"+str(sample_key)+"_"+str(hh_size)
        ...
        with np.load(os.getcwd()+"/output/Temp/Size/coefficients_nfnn_"+str(sample_key)+"_"+str(hh_size)+'.npz') as data:
            w_1_out = data['w_1']
            w_2_out = data['w_2']
            b_1_out = data['b_1']
            b_2_out = data['b_2']
        ...
        hh_size = 2 #Min cv cost of the current setup
        cv_accuracy, test_accuracy, w_1_out, w_2_out, b_1_out, b_2_out, hh_size = nfnn(hh_size, x_size, y_size, x_train, y_train, x_cv, y_cv, x_test, y_test, tol_iter, number_goods)
        
    return(cv_accuracy, test_accuracy, w_1_out, w_2_out, b_1_out, b_2_out)
