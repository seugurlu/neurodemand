...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep  2 11:06:38 2017
...
#Keep only prices, budget shares, and expenditures
prices = data.filter(regex = "^p.")
budget_shares = data.filter(regex = "^w.")
expenditures = data['total.expenditure']
input_data = prices.join(expenditures)

number_hh = input_data.shape[0]
number_goods = len(input_data.filter(regex = "^p").columns)

#Create Training and Test Samples
x_train, x_test, y_train, y_test = train_test_split(input_data, budget_shares, test_size=0.30, random_state=1)
x_cv, x_test, y_cv, y_test = train_test_split(x_test, y_test, test_size = 0.5, random_state = 2)

#%%
#AIDS Results
aids = aids.aids(x_train, y_train)
np.save('./output/AIDS/aids_main_sample', aids)

#%%
#QUAIDS Results
quaids = quaids.quaids(x_train, y_train)
np.save('./output/QUAIDS/quaids_main_sample', quaids)
...
mini_batch_grad_epoch_limit = 10000
penalty_limit = 50
sample_range = range(100)
batch_size = 200
zeta_multiplier = 1.5
sess = tf.InteractiveSession()
x_train_cfnn = np.log( nf.impose_homogeneity(x_train) )#Generate input matrix as homogeneity adjusted log
x_cv_cfnn = np.log( nf.impose_homogeneity(x_cv) )#Generate input matrix as homogeneity adjusted log
x_test_cfnn = np.log( nf.impose_homogeneity(x_test) )#Generate input matrix as homogeneity adjusted log
...
while hh_size <= hh_end:
    ...
    slutsky_cost = nf.slutsky_dif(length, x, yhat, w_1, w_2, z1, number_goods)
    train_cost = tf.add( tf.reduce_mean( (y - yhat)**2 ), tf.multiply(zeta,slutsky_cost) )
    ...
    for epoch in range(stochastic_grad_epoch_limit):
        ...
        for i in range( len(x_train) ):
            train_step.run({ x:x_train_cfnn[i:i+1], y:y_train[i:i+1], length: 1 })
        train_accuracy_post_optimization = train_cost.eval( {x: x_train_cfnn, y: y_train, length : len(x_train_cfnn)} )
    ...
    for epoch in range(mini_batch_grad_epoch_limit):
        ...
        for i in range( int( len(x_train_cfnn) / batch_size )): # Train with each example
            batch_x = x_train_cfnn[i*batch_size:(i+1)*batch_size]
        ...
        if remaining_observations != 0:
            batch_x = x_train_cfnn[len(x_train_cfnn)-remaining_observations:len(x_train_cfnn)]
    ...
    for epoch1 in range(penalty_limit):
        ...
        print("Stochastic gradient starts with hhsize: %s and zeta: %.2f" %(hh_size, zeta.eval()))
        for epoch2 in range(stochastic_grad_epoch_limit):
            train_accuracy_pre_optimization = train_accuracy_post_optimization
            for i in range( int( len(x_train_cfnn) / batch_size )): # Train with each example
                batch_x = x_train_cfnn[i*batch_size:(i+1)*batch_size]
                batch_y = y_train[i*batch_size:(i+1)*batch_size]
            ...
            if remaining_observations != 0:
                ...
                train_step.run( {x:batch_x, y:batch_y, length:remaining_observations} )
            train_accuracy_post_optimization = train_cost.eval( {x: x_train_cfnn, y: y_train, length : len(x_train_cfnn)} )
            print('Stochastic gradient continues at hhsize: %s, zeta: %.2f, epoch2 %s. Post_cost: %.8f' %(hh_size, zeta.eval(), epoch2, train_accuracy_post_optimization ) )
            if tf.abs( train_accuracy_pre_optimization - train_accuracy_post_optimization ).eval() <= tol_iter :
                print("Stochastic gradient converged")
                break
    ...
    train_acc = accuracy_cost.eval( {x: x_train_cfnn, y: y_train, length:len(x_train_cfnn)} )
    cv_acc = accuracy_cost.eval( {x: x_cv_cfnn, y: y_cv, length:len(x_cv_cfnn)} )
    test_acc = accuracy_cost.eval( {x: x_test_cfnn, y: y_test, length: len(x_test_cfnn)} )
    ...
    save_name = "./output/Temp/main_sample_cfnn/coefficients_cfnn_"+str(hh_size)
...
with np.load("./output/Temp/main_sample_cfnn/coefficients_cfnn_"+str(hh_size)+'.npz') as data:
    w_1 = data['w_1']
    w_2 = data['w_2']
    b_1 = data['b_1']
    b_2 = data['b_2']
...
np.savez("./output/NN/cfnn/pycoefficients_cfnn_main_sample",
