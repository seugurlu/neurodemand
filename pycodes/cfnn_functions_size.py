...
def cfnn(h_size, x_size, y_size, x_train, y_train, x_cv, y_cv, x_test, y_test, tol_iter, tol_coef, number_goods):  
    #4.1 PlaceHolders
    ...
    reg_coef = tf.constant(1, dtype = tf.float32) #Symmetry coefficient 1st element
    
    yhat, z1 = nf.forwardprop(x, w_1, w_2, b_1, b_2)
    slutsky_cost = nf.slutsky_dif(length, x, yhat, w_1, w_2, z1, number_goods)
    train_cost = tf.add( tf.reduce_mean( (y - yhat)**2 ), tf.multiply(reg_coef,slutsky_cost) )
    ...
    for epoch1 in range(500):
        reg_coef = reg_coef * 1.1
...
def main_with_cv(sample_key, x_train, y_train, x_cv, y_cv, x_test, y_test, tol_iter, tol_coef, number_goods):
    ...
    while hh_size <= hh_end:
        ...
        cv_accuracy, test_accuracy, w_1_out, w_2_out, b_1_out, b_2_out = cfnn(hh_size, x_size, y_size, x_train, y_train, x_cv, y_cv, x_test, y_test, tol_iter, tol_coef, number_goods)
        save_name = os.getcwd()+"/output/Temp/Size/coefficients_cfnn_size_"+str(sample_key)+"_"+str(hh_size)
    ...
    with np.load(os.getcwd()+"/output/Temp/Size/coefficients_cfnn_size_"+str(sample_key)+"_"+str(hh_size)+'.npz') as data:
        w_1_out = data['w_1']
        w_2_out = data['w_2']
        b_1_out = data['b_1']
        b_2_out = data['b_2']
